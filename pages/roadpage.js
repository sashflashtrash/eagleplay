  ...
  return (
    ...
    <div style={{ padding: 20, backgroundColor: darkMode ? '#111' : '#fff', color: darkMode ? '#eee' : '#000', position: 'relative' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h1 style={{ fontSize: '2rem' }}>Pässe · Routen · Touren</h1>
...
<div style={{ display: 'flex',
        ...
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} style={{ padding: '8px 12px', borderRadius: 8 }}>
          ...
          <option value="closed">{lang === 'DE' ? 'Geschlossen' : 'Closed'}</option>
        </select>
        ...
        <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} style={{ padding: '8px 12px', borderRadius: 8 }}>
          <option value="all">{lang === 'DE' ? 'Typ' : lang === 'FR' ? 'Type' : lang === 'IT' ? 'Tipo' : 'Type'}</option>
          {filteredTypes.map(t => (
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {lang === 'DE' ? 'Nur Favoriten' : 'Only Favorites'}
        </label>
      </div>
      ...
      {showHelp && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: '#000a', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ background: '#fff', padding: 30, borderRadius: 16, maxWidth: 600, width: '90%', color: '#000' }}>
            <h2>🧭 Was finde ich hier?</h2>
            <p>
              Diese Seite zeigt dir eine kuratierte Übersicht von Schweizer Pässen, Routen und Touren.
              Du kannst nach Namen suchen, nach Status, Region, Land und Typ filtern – oder nur deine Favoriten anzeigen lassen.
            </p>
            <p>Kantone sind in der Schweiz die Bundesländer.</p>
            <p>Level 1 = breite Straße mit Mittellinie.</p>
            <p>Level 2 = normale Straße, teils ohne Mittellinie.</p>
            <p>Level 3 = schmale Straße, für große Fahrzeuge ungeeignet.</p>
